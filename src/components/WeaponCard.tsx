...
export default function WeaponCard({ weapon }: Props) {
  return (
    <Link href={`/build?weapon=${encodeURIComponent(weapon.name)}`} className="group relative w-full max-w-60 mx-auto">
  ...
  bg-[radial-gradient(circle,rgba(255,230,150,0.18)_0%,rgba(255,255,255,0.08)_40%,rgba(0,0,0,0)_70%)]">
        ...
        <div className="mt-2 text-center text-xs text-gray-400 space-y-1">
          <p>
            Тип: <span className="text-gray-200">{weapon.type}</span>
          </p>
          <p>
            Урон: <span className="text-gray-200">{weapon.damage}</span>
          </p>
        </div>
